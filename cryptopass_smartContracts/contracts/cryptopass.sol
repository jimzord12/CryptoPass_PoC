...
interface ICryptoPass {
    enum Role { None, Student, Professor, Secretary, Admin }
...
contract AccessToken is ERC721, Ownable {
    ...
    constructor(address _cryptoPassAddress) ERC721("CryptoPassAccessToken", "CPATK") {
    ...
    function mintToken(address to) public returns (uint256) {
        ...
        require(cryptoPassContract.hasSBT(), "CPATK: Caller Does not possess an SBT");
        if (balanceOf(to) > 0) { // Does already have a AccessToken?
        // Yes, the user has AT...
        uint userAccessTokenID = getTokenId(to); // Let's grab its ID;
        require(_isTokenExpired(userAccessTokenID), "CPATK: Token has NOT expired yet."); // It must be expired to continue
        // Here, we know that the user has an expired AT...
         _burn(userAccessTokenID); // So let's burn it!
         // So a new one can be created...
        } 

        require(balanceOf(to) == 0, "CPATK: User already has an Unused and Unexpired AccessToken");
    ...
    function changeCryptoPassAddress(address _newContractAddr) onlyOwner external {
    ...
    function useToken(uint256 tokenId) external {
        ...
        require(uint(cryptoPassContract.getUserRole(msg.sender)) > 1, "CPATK: Insufficient Access Level"); // Restricting Access
    ...
    function getTokenId(address owner) public view returns (uint256) {
        require(uint(cryptoPassContract.getUserRole(msg.sender)) > 2, "CPATK: Insufficient Access Level"); // Restricting Access
